...
- 시도당 수천 건이라 **페이지네이션이 핵심**(`paginateAll`): 페이지 크기를 옵션
  최댓값으로 키운 뒤, 보이는 번호(1~10)를 모두 누르고 → 번호줄 아래 **'다음'
  버튼(`w2pageList_control_next`)**으로 다음 묶음을 펼쳐 또 누르고 → 비활성/진전
  없음까지 반복해요. ('끝(control_last)' 버튼은 중간을 건너뛰니 안 써요.)
- '다음' 버튼 셀렉터는 class/title/aria로 폭넓게 잡아요(`nextBtnSel`). 못 잡으면
  `⚠ 페이지네이션 누락 가능 — N/M` 경고가 떠요. 그때 실제 DOM의 class/title을
  보고 `nextBtnSel`만 고치면 돼요.
